...
class RecipyPage:
    ...
    def show_recipy(self):
        ...
        self.titre_recette = Label(self.recette_frame, text=self.infos[1], font=("MV Boli", 32),
        ...
        for i in range(len(etapes) + len(self.ingredients)):
            if i == 0:
                self.ing_titre = Label(self.recette_frame, text="Ingrédients:", font=("MV Boli", 18),
                                       background=self.bg_color)
                self.background_widgets.append(self.ing_titre)
                self.ing_titre.pack(side=TOP, fill=BOTH, expand=1)
            elif i == len(self.ingredients):
                self.etp_titre = Label(self.recette_frame, text="Étapes:", font=("MV Boli", 18),
                                       background=self.bg_color)
